...
class CineTimelineWidget {
  ...
  renderGlobalSettings() {
    ...
    fields.append(el("div", "cine-note", "H3 三字段可以分布在全局与片段提示词中，生成时会自动重组。可在全局 integrated_multimodal_description 内使用 {{segment_prompt}}（或 {{片段提示词}}）精确指定片段提示词的插入位置；未使用占位符时自动追加到该字段末尾。"));
  ...
  renderMusicSettings(music) {
    ...
    fields.append(el("div", "cine-note", "背景音乐片段的位置和边界按 0.1 秒吸附；片段可相互叠加混音，不会替换视频原生对白和环境声。"));
  ...
  renderSegmentSettings(shot) {
    ...
    fields.append(el("div", "cine-note", "采样方案只影响当前片段；串联完整影片不会再次放大或采样。"));
    fields.append(el(
      "div", "cine-note",
      isFirst
        ? "首个片段没有视觉参考时自动使用纯文生。"
        : "“视频延长”读取上一片段当前激活版本的连续画面与声音 latent，并自动生成、匹配和裁切约 22 帧重叠区。"
    ));
    fields.append(this.renderReferenceEditor("shot", shot));
    fields.append(el("div", "cine-note", "直接切换会独立生成；视频延长每次生成时读取上一片段当前激活版本的最新续接 latent。"));
    const continuity = this.automaticContinuity(shot);
    if (continuity.lineageStale) {
      fields.append(el(
  ...
  renderReferenceEditor(scope, shot) {
    ...
    body.append(el(
      ...
      "cine-note",
      isFL2VA
        ? "单图生视频：只加入一张图片时自动作为首帧；第二张自动作为尾帧。也可以手动把唯一图片改为尾帧，执行 L2VA。"
        : "Ref2VA 支持单个图片或单个视频参考，不要求至少两个参考；图片、视频和音频分别独立编号。"
    ));
